...
const dialogShowReportOnTable = document.getElementById(
  "dialogShowReportOnTable"
);
...
rad_aton_all.addEventListener("click", (e) => {
  for (let i in lst_atoninfo) {
    ...
    if (
      chk_select_beacon.checked &&
      atonInfo["type"].toLowerCase() == "beacon" &&
      chkElem == null
    ) {
      marker.addTo(map);
    }
    ...
    if (
      chk_select_buoy.checked &&
      atonInfo["type"].toLowerCase() == "buoy" &&
      chkElem == null
    ) {
      marker.addTo(map);
    }
    ...
    if (
      chk_select_lighthouse.checked &&
      atonInfo["type"].toLowerCase() == "lighthouse" &&
      chkElem == null
    ) {
      marker.addTo(map);
    }
...
mapboxgl.accessToken =
  "pk.eyJ1IjoiYXp6dWxoaXNoYW0iLCJhIjoiY2s5bjR1NDBqMDJqNDNubjdveXdiOGswYyJ9.SYlfXRzRtpbFoM2PHskvBg";
...
function showVesselPopup(e) {
  ...
  if (!toggleBtn.classList.contains("open")) {
    const description =
      '<div class="text-gray-900"><h4 class="font-bold">' +
      elemid_text[1] +
      "</h4>" +
      "<small> Structure &nbsp;: " +
      get_atoninfo["type"] +
      ...
      "</small><br>" +
      "<small> ATON Batt &nbsp;: " +
      get_atoninfo["volt_int"].toFixed(2) +
      "</small><br>" +
      "<small> Lantern Batt &nbsp;: " +
      get_atoninfo["volt_ex1"].toFixed(2) +
      "</small><br>" +
      "<small> Off Position &nbsp;: " +
      (get_atoninfo["off_pos"] == 0 ? "OK" : "NG") +
      "</small><br>" +
      "<small> Ambient &nbsp;: " +
      ambient[get_atoninfo["ambient"]] +
      "</small><br>" +
      "<small> Light &nbsp;: " +
      lightstatus[get_atoninfo["light"]] +
      "</small><br>" +
      "<small> local Time &nbsp;: " +
      get_atoninfo["lcl_ts"] +
      "</small><br>" +
...
function vessel_info_panel(mmsi) {
  ...
  if (rms) {
    ...
    const det_positionAccuracyDesc = document.getElementById(
      "det-positionAccuracyDesc"
    );
    ...
    det_positionAccuracyDesc.innerText =
      ": " + get_atoninfo["positionAccuracyDesc"];
    ...
    if ((dac == 533 && fid == 1) || (dac == 235 && fid == 10)) {
      ...
      const det_buoy_led_power = document.getElementById(
        "det-buoy-led-power-fid1"
      );
      const det_buoy_low_vin = document.getElementById("det-buoy-low-vin-fid1");
      const det_buoy_photocell = document.getElementById(
        "det-buoy-photocell-fid1"
      );
      const det_buoy_temp = document.getElementById("det-buoy-temp-fid1");

      detShiptype.innerText = ": " + get_atoninfo["type"];
      detShipname.innerText =
        ": " +
        (get_atoninfo["atonname"].length > 15
          ? get_atoninfo["atonname"].substring(0, 15) + "..."
          : get_atoninfo["atonname"]);
      detMMSI.innerText = ": " + get_atoninfo["mmsi"];
      ...
      det_health.innerText =
        ": " + (get_atoninfo["health"] == 0 ? "Good" : "Alarm");

      det_beat.innerText = ": " + (get_atoninfo["beat"] == 0 ? "Tick" : "Tock");
      det_alarm_active.innerText =
        ": " + lantern_batt[get_atoninfo["buoy_led_power"]];
      det_buoy_led_power.innerText =
        ": " + lantern[get_atoninfo["buoy_low_vin"]];
      det_buoy_low_vin.innerText = ": " + ambient[get_atoninfo["ambient"]];
      det_buoy_photocell.innerText =
        ": " + (get_atoninfo["buoy_photocell"] == 0 ? "Close" : "Open");
      det_buoy_temp.innerText =
        ": " + (get_atoninfo["off_pos"] == 0 ? "On Pos." : "Off Pos.");
    }
    ...
    if (dac == 533 && fid == 2) {
      ...
      const det_buoy_led_power = document.getElementById(
        "det-buoy-led-power-fid2"
      );
      const det_buoy_low_vin = document.getElementById("det-buoy-low-vin-fid2");
      const det_buoy_photocell = document.getElementById(
        "det-buoy-photocell-fid2"
      );
      const det_buoy_temp = document.getElementById("det-buoy-temp-fid2");
      const det_buoy_force_off = document.getElementById(
        "det-buoy-force-off-fid2"
      );
      const det_buoy_islight = document.getElementById("det-buoy-islight-fid2");
      const det_buoy_errled_short = document.getElementById(
        "det-buoy-errled-short-fid2"
      );
      const det_buoy_errled_open = document.getElementById(
        "det-buoy-errled-open-fid2"
      );
      const det_buoy_errled_voltlow = document.getElementById(
        "det-buoy-errled-voltlow-fid2"
      );
      const det_buoy_errled_vinlow = document.getElementById(
        "det-buoy-errled-vinlow-fid2"
      );
      const det_buoy_errled_power = document.getElementById(
        "det-buoy-errled-power-fid2"
      );
      const det_buoy_adjmaxpower = document.getElementById(
        "det-buoy-adjmaxpower-fid2"
      );
      ...
      det_beat.innerText = ": " + (get_atoninfo["beat"] == 0 ? "Tick" : "Tock");
      det_alarm_active.innerText =
        ": " + (get_atoninfo["alarm_active"] == 0 ? "Normal" : "Fail");
      det_buoy_led_power.innerText =
        ": " + (get_atoninfo["buoy_led_power"] == 0 ? "Off" : "On");
      det_buoy_low_vin.innerText =
        ": " + (get_atoninfo["buoy_low_vin"] == 0 ? "Normal" : "Fail");
      det_buoy_photocell.innerText =
        ": " + (get_atoninfo["buoy_photocell"] == 0 ? "Off" : "On");
      det_buoy_temp.innerText =
        ": " + (get_atoninfo["buoy_temp"] == 0 ? "Normal" : "Fail");
      det_buoy_force_off.innerText =
        ": " + (get_atoninfo["buoy_force_off"] == 0 ? "Off" : "On");
      det_buoy_islight.innerText =
        ": " + (get_atoninfo["buoy_islight"] == 0 ? "Off" : "On");
      det_buoy_errled_short.innerText =
        ": " + (get_atoninfo["buoy_errled_short"] == 0 ? "Normal" : "Fail");
      det_buoy_errled_open.innerText =
        ": " + (get_atoninfo["buoy_errled_open"] == 0 ? "Off" : "On");
      det_buoy_errled_voltlow.innerText =
        ": " + (get_atoninfo["buoy_errled_voltlow"] == 0 ? "Normal" : "Fail");
      det_buoy_errled_vinlow.innerText =
        ": " + (get_atoninfo["buoy_errled_vinlow"] == 0 ? "Close" : "Open");
      det_buoy_errled_power.innerText =
        ": " + (get_atoninfo["buoy_errled_power"] == 0 ? "Off" : "On");
      det_buoy_adjmaxpower.innerText =
        ": " + (get_atoninfo["buoy_adjmaxpower"] == 0 ? "Normal" : "Fail");
    }
    ...
    if (dac == 533 && fid == 4) {
      ...
      const det_buoy_errled_short = document.getElementById(
        "det-buoy-errled-short"
      );
      const det_buoy_errled_open = document.getElementById(
        "det-buoy-errled-open"
      );
      const det_buoy_errled_voltlow = document.getElementById(
        "det-buoy-errled-voltlow"
      );
      const det_buoy_errled_vinlow = document.getElementById(
        "det-buoy-errled-vinlow"
      );
      const det_buoy_errled_power = document.getElementById(
        "det-buoy-errled-power"
      );
      const det_buoy_adjmaxpower = document.getElementById(
        "det-buoy-adjmaxpower"
      );
      const det_buoy_sensor_interrupt = document.getElementById(
        "det-buoy-sensor-interrupt"
      );
      const det_buoy_solarcharging = document.getElementById(
        "det-buoy-solarcharging"
      );
      ...
      det_beat.innerText = ": " + (get_atoninfo["beat"] == 0 ? "Tick" : "Tock");
      det_alarm_active.innerText =
        ": " + (get_atoninfo["alarm_active"] == 0 ? "No" : "Yes");
      det_buoy_led_power.innerText =
        ": " + (get_atoninfo["buoy_led_power"] == 0 ? "No" : "Yes");
      det_buoy_low_vin.innerText =
        ": " + (get_atoninfo["buoy_low_vin"] == 0 ? "No" : "Yes");
      det_buoy_photocell.innerText =
        ": " + (get_atoninfo["buoy_photocell"] == 0 ? "No" : "Yes");
      det_buoy_temp.innerText =
        ": " + (get_atoninfo["buoy_temp"] == 0 ? "No" : "Yes");
      det_buoy_force_off.innerText =
        ": " + (get_atoninfo["buoy_force_off"] == 0 ? "No" : "Yes");
      det_buoy_islight.innerText =
        ": " + (get_atoninfo["buoy_islight"] == 0 ? "No" : "Yes");
      det_buoy_errled_short.innerText =
        ": " + (get_atoninfo["buoy_errled_short"] == 0 ? "No" : "Yes");
      det_buoy_errled_open.innerText =
        ": " + (get_atoninfo["buoy_errled_open"] == 0 ? "No" : "Yes");
      det_buoy_errled_voltlow.innerText =
        ": " + (get_atoninfo["buoy_errled_voltlow"] == 0 ? "No" : "Yes");
      det_buoy_errled_vinlow.innerText =
        ": " + (get_atoninfo["buoy_errled_vinlow"] == 0 ? "No" : "Yes");
      det_buoy_errled_power.innerText =
        ": " + (get_atoninfo["buoy_errled_power"] == 0 ? "No" : "Yes");
      det_buoy_adjmaxpower.innerText =
        ": " + (get_atoninfo["buoy_adjmaxpower"] == 0 ? "No" : "Yes");
      det_buoy_sensor_interrupt.innerText =
        ": " + (get_atoninfo["buoy_sensor_interrupt"] == 0 ? "No" : "Yes");
      det_buoy_solarcharging.innerText =
        ": " + (get_atoninfo["buoy_solarcharging"] == 0 ? "No" : "Yes");
    }
  }
}
...
function summary_details(obj) {
  ...
  cnt_in_nomsg6.innerText =
    obj["no_msg6_cnt"] + " (" + obj["no_msg6_cnt_p"] + ")";
  cnt_in_offpos.innerText =
    obj["offpos_cnt"] + " (" + obj["offpos_cnt_p"] + ")";
  cnt_in_light.innerText = obj["light_cnt"] + " (" + obj["light_cnt_p"] + ")";
  cnt_in_ldr.innerText = obj["ldr_cnt"] + " (" + obj["ldr_cnt_p"] + ")";
  cnt_in_battAton.innerText =
    obj["battAton_cnt"] + " (" + obj["battAton_cnt_p"] + ")";
  cnt_in_battLant.innerText =
    obj["battLant_cnt"] + " (" + obj["battLant_cnt_p"] + ")";
}

function computeSummaryDetails(atonInfo, data) {
  data.atons_cnt += 1;
...
function cal_summary_details() {
  ...
  if (
    ...
    chk_select_buoy.checked &&
    chk_select_lighthouse.checked &&
    rad_aton_all.checked
  ) {
    summary_details(summary_det);

    for (let i in lst_atoninfo) {
      atonInfo = lst_atoninfo[i];
      summary_data = computeSummaryDetails(atonInfo, summary_data);
    }
  } else {
    if (
      chk_select_beacon.checked &&
    ...
    aton_summary = {
      ...
      no_msg6_cnt_p:
        ((summary_data.no_msg6_cnt / summary_data.atons_cnt) * 100).toFixed(2) +
        "%",
      light_cnt: summary_data.light_err_cnt,
      light_cnt_p:
        ((summary_data.light_err_cnt / summary_data.atons_cnt) * 100).toFixed(
          2
        ) + "%",
      battAton_cnt: summary_data.battAton_cnt,
      battAton_cnt_p:
        ((summary_data.battAton_cnt / summary_data.atons_cnt) * 100).toFixed(
          2
        ) + "%",
      battLant_cnt: summary_data.battLant_cnt,
      battLant_cnt_p:
        ((summary_data.battLant_cnt / summary_data.atons_cnt) * 100).toFixed(
          2
        ) + "%",
      offpos_cnt: summary_data.offpos_cnt,
      offpos_cnt_p:
        ((summary_data.offpos_cnt / summary_data.atons_cnt) * 100).toFixed(2) +
        "%",
      ldr_cnt: summary_data.ldr_cnt,
      ldr_cnt_p:
        ((summary_data.ldr_cnt / summary_data.atons_cnt) * 100).toFixed(2) +
        "%",
    };
...
function update_aton_on_map(elem, dataNo) {
  ...
  for (let i in lst_atoninfo) {
    ...
    if (selected_aton == undefined) {
      ...
      let chkElem = document.getElementById(
        "vessels-on-sea_" + atonInfo["mmsi"]
      );
      ...
      if (obj["ss_messageType"] == 21 && obj["ss_rowcountby_mmsi"] > 0) {
        ...
        if (lat > -90 && lat <= 90) {
          ...
          if (get_mmsi === undefined) {
            ...
            el.id =
              type.toLowerCase() == "lighthouse" ? "aton-lighthouse" : "aton";
  ...
  Highcharts.chart(chartDivId, {
    ...
    title: {
      text:
        lst_voltdata[0].mmsi +
        " - " +
        lst_voltdata[0].atonname +
        "<br>ATON Battery Voltage and Lantern Voltage for last 7 days",
      align: "center",
    },
